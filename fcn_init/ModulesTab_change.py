...
def onTabChanged(self):
    ...
    if hasattr(self, 'Tab_index') and self.Tab_index == 4 and self.tabModules.currentIndex() != 4:
        ...
        # Set all axis unchecked if present
        for axis_name in ['check_axis_X', 'check_axis_Y', 'check_axis_Z']:
            check = getattr(self, axis_name, None)
    ...
    if hasattr(self, 'Tab_index') and self.Tab_index != 4 and self.tabModules.currentIndex() == 4:
        ph_folder = getattr(self, 'PhOperFolder', None)
        folder_text = ph_folder.text() if ph_folder is not None else ''
        if not os.path.isdir(folder_text):
            QMessageBox.warning(None, "Warning", f"No input folder on device with path: {folder_text}.\nSet the correct folder with GCODE and CSV files")
            self.Tab_index = 0
            ...
            self.Tab_index = self.tabModules.currentIndex()
            if hasattr(self, 'MoVeSpeedFactor') and self.MoVeSpeedFactor is not None:
